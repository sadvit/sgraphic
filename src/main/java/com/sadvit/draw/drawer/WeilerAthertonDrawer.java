...
public class WeilerAthertonDrawer implements Drawer {
    ...
    private void clip(SimpleCanvas canvas) {
        LinkedList<Node> _windowPoints = deepClone(windowPoints);
        LinkedList<Node> _clipPoints = deepClone(clipPoints);
        intersect(windowPoints, clipPoints);
        intersect(_clipPoints, _windowPoints);
    ...
    private static void intersect(LinkedList<Node> subject, LinkedList<Node> clip) {
        ...
        for (int i = 0; i < subject.size() - 1; i++) {
            ...
            if (i > 0 && current.equals(subject.get(0)))
                break;
            ...
            int position = result.indexOf(next);
            result.addAll(position, sublist);
        }
        subject.clear();
        subject.addAll(result);
    }
